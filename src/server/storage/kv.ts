 ...
 */
export function getCachedHealthData(): Promise<CachedHealthData> {
  return Promise.resolve(structuredClone(healthCache));
}

/**
 * Update health data for a specific app or plugin
 */
export function updateHealthData(request: UpdateHealthRequest): Promise<{
  success: boolean;
  storage: string;
  timestamp: string;
  error?: string;
  ...
  try {
    ...
    if (request.type === "app") {
      healthCache.apps[request.key] = request.result;
    } else {
      healthCache.plugins[request.key] = request.result;
    }
